...
This document covers the current startup flow implemented in `launch.py`, the AprilTag WebSocket server in `marker_to_pos/server.py`, and the Flask backend in `web/app.py`.
...
Debug capture settings live in `assets/registration/config.yml`:
